...
public class InstalledEntry {
    ...
    public String getName(WebOSConnection webOS) {
        if(name.equals("This is a webOS application.") || name.equals("Unknown")) {
            String ctrl = webOS.offlineRoot + "/usr/lib/ipkg/info/" + id + ".control";
            if(webOS.getPkgMgr().equals("opkg")) {
                ctrl = webOS.offlineRoot + "/var/lib/opkg/info/" + id + ".control";
            }
            if(webOS.fileExists(ctrl)) {
                try {
                    String out = webOS.runProgram("/bin/grep", new String[] {"-e",
                            "Description:", ctrl});
                    if(out.startsWith("Description:")) {
                        name = out.substring(out.indexOf(":")+1).trim();
                    }
                } catch(Exception e) {}
            }
            if(webOS.fileExists("/media/cryptofs/apps/usr/palm/applications/" + id +
                    "/appinfo.json")) {
                try {
