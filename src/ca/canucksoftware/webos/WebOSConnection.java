...
public abstract class WebOSConnection {
     ...
     public ArrayList<InstalledEntry> listInstalled() {
        ...
        try {
            String stdout = runProgram("/bin/grep", new String[] {"-e", "Package:",
                    "-e", "Version:", "-e", "Description:", offlineRoot +
                    "/usr/lib/ipkg/status"}); //"-e", "^$"
            String[] lines = stdout.split("\n");
            for(int i=0; i<lines.length; i++) {
                if(lines[i].length()>0) {
                    String id = lines[i].substring(lines[i].indexOf(":")+1).trim();
                    i++;
                    String version = lines[i].substring(lines[i].indexOf(":")+1).trim();
                    i++;
                    String name = lines[i].substring(lines[i].indexOf(":")+1).trim();
    ...
    public void configIpkg() {
        if(!fileExists("/media/cryptofs/apps/etc/ipkg/arch.conf")) {
            try {
                mkdir("/media/cryptofs/apps/etc/ipkg");
                runProgram("/bin/cp", new String[]{"-f", "/etc/ipkg/arch.conf",
                        "/media/cryptofs/apps/etc/ipkg/arch.conf"});
            } catch (Exception e) {
                System.err.println("Unable to copy ipkg configuration file:" +
                        e.getMessage());
            }
        }
    }
    ...
    public boolean install(String filename) {
        ...
        try {
            ...
            try {
                ...
                if(fileExists(offlineRoot + "/usr/lib/ipkg/info/" + appid + ".control")) {
                    if(fileExists(offlineRoot + "/usr/lib/ipkg/info/" + appid + ".postinst")) {
                        out = runIpkgScript(offlineRoot + "/usr/lib/ipkg/info/" + appid +
                                ".postinst");
                    }
                    result = true;
    ...
    public boolean uninstall(String appid) {
        ...
        try {
            ...
            String restartFlag = "";
            try {
                out = runProgram("/bin/grep", new String[] {"Source:",
                        offlineRoot + "/usr/lib/ipkg/info/" + appid + ".control"});
                JSONObject source = new JSONObject(out.substring(out.indexOf("{"),
                        out.lastIndexOf("}")+1));
                restartFlag = source.getString("PostRemoveFlags").toLowerCase();
            } catch(Exception e) {}
            if(fileExists(offlineRoot + "/usr/lib/ipkg/info/" + appid + ".prerm")) {
                out = runIpkgScript(offlineRoot + "/usr/lib/ipkg/info/" + appid +
                        ".prerm");
            }
            if(fileExists(offlineRoot + "/usr/palm/applications/" + appid +
                ...
                try {
                    out = runProgram("/usr/bin/ipkg", new String[] {"-o", offlineRoot,
            ...
            if(!fileExists(offlineRoot + "/usr/lib/ipkg/info/" + appid + ".control")) {
                result = true;
                if(appid.startsWith("ca.canucksoftware.patches.")) {
